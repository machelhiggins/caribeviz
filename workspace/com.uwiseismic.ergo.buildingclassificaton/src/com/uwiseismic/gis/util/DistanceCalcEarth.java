...
public class DistanceCalcEarth {
    ...
    public static  WGS84BoundingBox createBBox(double lat, double lon, double radiusInMeter) {
        if (radiusInMeter <= 0)
        ...
        return new WGS84BoundingBox(lon - dLon, lat - dLat, 2*dLon,2*dLat);
    }
    ...
    public static double calcShrinkFactor(double a_lat_deg, double b_lat_deg) {
        return cos(toRadians((a_lat_deg + b_lat_deg) / 2));
    }

    public static SimplePoint calcCrossingPointToEdge(double r_lat_deg, double r_lon_deg,
                                           double a_lat_deg, double a_lon_deg,
                                           double b_lat_deg, double b_lon_deg) {
        double shrinkFactor = calcShrinkFactor(a_lat_deg, b_lat_deg);
        double a_lat = a_lat_deg;
        ...
        if (delta_lat == 0)
            // special case: horizontal edge
            return new SimplePoint(a_lat_deg, r_lon_deg);

        if (delta_lon == 0)
            // special case: vertical edge        
            return new SimplePoint(r_lat_deg, a_lon_deg);
        ...
        double c_lon = a_lon + factor * delta_lon;
        double c_lat = a_lat + factor * delta_lat;
        return new SimplePoint(c_lat, c_lon / shrinkFactor);
    }
    ...
    public static SimplePoint projectCoordinate(double latInDeg, double lonInDeg, double distanceInMeter, double headingClockwiseFromNorth) {
        ...
        return new SimplePoint(projectedLat, projectedLon);
    }

    public static boolean isCrossBoundary(double lon1, double lon2) {
