...
/**
 * Same as {@link ObjectByteHashMap} but does not implement per-instance
...
public class ObjectByteScatterMap<KType> extends ObjectByteHashMap<KType>
{
  ...
  public ObjectByteScatterMap(int expectedElements, double loadFactor) {
   ...
   */
  public static <KType> ObjectByteScatterMap<KType> from(KType[] keys, byte[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    ObjectByteScatterMap<KType> map = new ObjectByteScatterMap<>(keys.length);
