...
public class ObjectArrayDeque<KType> 
   ...
   */
  private void descendingForEach(ObjectProcedure<? super KType> procedure, int fromIndex, final int toIndex) {
   ...
   */
  private void descendingForEach(ObjectPredicate<? super KType> predicate, int fromIndex, final int toIndex) {
    if (fromIndex == toIndex)
      return;
  ...
  /**
   * Compare order-aligned elements against another {@link ObjectDeque}.
   * Equality comparison is performed with this object's {@link #equals(Object, Object)} 
   * method.
   */
  protected boolean equalElements(ObjectArrayDeque<?> other) {
    ...
    while (i1.hasNext() && i2.hasNext()) {
      if (!this.equals(i2.next().value,  i1.next().value)) {
        return false;
      }
    }
