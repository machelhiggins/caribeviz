...
/**
 * A hash map of <code>long</code> to <code>Object</code>, implemented using open
             ...
             Cloneable
{
  ...
  public int removeAll(LongObjectPredicate<? super VType> predicate) {
    ...
    if (hasEmptyKey) {
      if (predicate.apply(0L, (VType) values[mask + 1])) {
        hasEmptyKey = false;
        values[mask + 1] = null;
      }
    }
  ...
  public <T extends LongObjectProcedure<? super VType>> T forEach(T procedure) {
    ...
    if (hasEmptyKey) {
      procedure.apply(0L, (VType) values[mask + 1]);
    }
  ...
  public final class KeysContainer extends AbstractLongCollection 
    ...
    private final LongObjectHashMap<VType> owner = LongObjectHashMap.this;
    ...
    public <T extends LongProcedure> T forEach(final T procedure) {
      owner.forEach(new LongObjectProcedure<VType>() {
        @Override
    ...
    public <T extends LongPredicate> T forEach(final T predicate) {
      owner.forEach(new LongObjectPredicate<VType>() {
        @Override
  ...
  private final class ValuesContainer extends AbstractObjectCollection<VType> {
    ...
    public int removeAll(final VType e) {
      return owner.removeAll(new LongObjectPredicate<VType>() {
        @Override
        public boolean apply(long key, VType value) {
