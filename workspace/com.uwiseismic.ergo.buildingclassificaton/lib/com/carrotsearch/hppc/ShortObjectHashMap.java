...
/**
 * A hash map of <code>short</code> to <code>Object</code>, implemented using open
             ...
             Cloneable
{
   ...
   */
  public ShortObjectHashMap(ShortObjectAssociativeContainer<? extends VType> container) {
    this(container.size());
    putAll(container);
  }
  ...
  public int removeAll(ShortObjectPredicate<? super VType> predicate) {
    ...
    if (hasEmptyKey) {
      if (predicate.apply(((short) 0), (VType) values[mask + 1])) {
        hasEmptyKey = false;
        values[mask + 1] = null;
      }
    }
  ...
  public <T extends ShortObjectProcedure<? super VType>> T forEach(T procedure) {
    ...
    if (hasEmptyKey) {
      procedure.apply(((short) 0), (VType) values[mask + 1]);
    }
  ...
  public final class KeysContainer extends AbstractShortCollection 
    ...
    private final ShortObjectHashMap<VType> owner = ShortObjectHashMap.this;
    ...
    public <T extends ShortProcedure> T forEach(final T procedure) {
      owner.forEach(new ShortObjectProcedure<VType>() {
        @Override
    ...
    public <T extends ShortPredicate> T forEach(final T predicate) {
      owner.forEach(new ShortObjectPredicate<VType>() {
        @Override
  ...
  private final class ValuesContainer extends AbstractObjectCollection<VType> {
    ...
    public int removeAll(final VType e) {
      return owner.removeAll(new ShortObjectPredicate<VType>() {
        @Override
        public boolean apply(short key, VType value) {
  ...
  public ShortObjectHashMap<VType> clone() {
    try {
      /*  */
      ShortObjectHashMap<VType> cloned = (ShortObjectHashMap<VType>) super.clone();
   ...
   */
  public static <VType> ShortObjectHashMap<VType> from(short[] keys, VType[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    ShortObjectHashMap<VType> map = new ShortObjectHashMap<>(keys.length);
