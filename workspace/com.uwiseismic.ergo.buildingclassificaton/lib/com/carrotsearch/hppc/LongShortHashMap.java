             ...
             Cloneable
{
   ...
   *          expansion (inclusive).
   */
  public LongShortHashMap(int expectedElements) {
   ...
   */
  public LongShortHashMap(int expectedElements, double loadFactor) {
   ...
   *          consequences.
   */
  public LongShortHashMap(int expectedElements, double loadFactor, HashOrderMixingStrategy orderMixer) {
   ...
   */
  protected boolean equalElements(LongShortHashMap other) {
    if (other.size() != size()) {
      return false;
    }

    for (LongShortCursor c : other) {
      long key =  c.key;
      if (!containsKey(key) ||
          !((get(key)) == (c.value))) {
        return false;
  ...
  public final class KeysContainer extends AbstractLongCollection 
    ...
    private final LongShortHashMap owner = LongShortHashMap.this;
    ...
    public <T extends LongProcedure> T forEach(final T procedure) {
      owner.forEach(new LongShortProcedure() {
        @Override
    ...
    public <T extends LongPredicate> T forEach(final T predicate) {
      owner.forEach(new LongShortPredicate() {
        @Override
  ...
  private final class ValuesContainer extends AbstractShortCollection {
    ...
    public int removeAll(final short e) {
      return owner.removeAll(new LongShortPredicate() {
        @Override
        public boolean apply(long key, short value) {
          return ((value) == (e));
        }
  ...
  public LongShortHashMap clone() {
    try {
      /*  */
      LongShortHashMap cloned = (LongShortHashMap) super.clone();
   ...
   */
  public static  LongShortHashMap from(long[] keys, short[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    LongShortHashMap map = new LongShortHashMap(keys.length);
   ...
   */
  protected void rehash(long[] fromKeys, short[] fromValues) {
   ...
   */
  protected void allocateThenInsertThenRehash(int slot, long pendingKey, short pendingValue) {
