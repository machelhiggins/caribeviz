             ...
             Cloneable
{
   ...
   *          expansion (inclusive).
   */
  public ShortCharHashMap(int expectedElements) {
   ...
   */
  public ShortCharHashMap(int expectedElements, double loadFactor) {
   ...
   *          consequences.
   */
  public ShortCharHashMap(int expectedElements, double loadFactor, HashOrderMixingStrategy orderMixer) {
   ...
   */
  public ShortCharHashMap(ShortCharAssociativeContainer container) {
    this(container.size());
    putAll(container);
  }
   ...
   */
  @Override
  public char addTo(short key, char incrementValue)
  {
   ...
   */
  protected boolean equalElements(ShortCharHashMap other) {
    if (other.size() != size()) {
      return false;
    }

    for (ShortCharCursor c : other) {
      short key =  c.key;
      if (!containsKey(key) ||
          !((get(key)) == (c.value))) {
        return false;
  ...
  public final class KeysContainer extends AbstractShortCollection 
    ...
    private final ShortCharHashMap owner = ShortCharHashMap.this;
    ...
    public <T extends ShortProcedure> T forEach(final T procedure) {
      owner.forEach(new ShortCharProcedure() {
        @Override
    ...
    public <T extends ShortPredicate> T forEach(final T predicate) {
      owner.forEach(new ShortCharPredicate() {
        @Override
  ...
  private final class ValuesContainer extends AbstractCharCollection {
    ...
    public int removeAll(final char e) {
      return owner.removeAll(new ShortCharPredicate() {
        @Override
        public boolean apply(short key, char value) {
          return ((value) == (e));
        }
  ...
  public ShortCharHashMap clone() {
    try {
      /*  */
      ShortCharHashMap cloned = (ShortCharHashMap) super.clone();
   ...
   */
  public static  ShortCharHashMap from(short[] keys, char[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    ShortCharHashMap map = new ShortCharHashMap(keys.length);
   ...
   */
  protected void rehash(short[] fromKeys, char[] fromValues) {
   ...
   */
  protected void allocateThenInsertThenRehash(int slot, short pendingKey, char pendingValue) {
