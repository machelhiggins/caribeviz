...
/**
 * Same as {@link ShortIntHashMap} but does not implement per-instance
...
public class ShortIntScatterMap extends ShortIntHashMap
{
  ...
  public ShortIntScatterMap(int expectedElements, double loadFactor) {
   ...
   */
  public static  ShortIntScatterMap from(short[] keys, int[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    ShortIntScatterMap map = new ShortIntScatterMap(keys.length);
