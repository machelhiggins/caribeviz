             ...
             Cloneable
{
   ...
   *          consequences.
   */
  public CharByteHashMap(int expectedElements, double loadFactor, HashOrderMixingStrategy orderMixer) {
  ...
  public final class KeysContainer extends AbstractCharCollection 
    ...
    public <T extends CharProcedure> T forEach(final T procedure) {
      owner.forEach(new CharByteProcedure() {
        @Override
    ...
    public <T extends CharPredicate> T forEach(final T predicate) {
      owner.forEach(new CharBytePredicate() {
        @Override
  ...
  private final class ValuesContainer extends AbstractByteCollection {
    ...
    public int removeAll(final byte e) {
      return owner.removeAll(new CharBytePredicate() {
        @Override
        public boolean apply(char key, byte value) {
          return ((value) == (e));
        }
  ...
  public CharByteHashMap clone() {
    try {
      /*  */
      CharByteHashMap cloned = (CharByteHashMap) super.clone();
   ...
   */
  public static  CharByteHashMap from(char[] keys, byte[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    CharByteHashMap map = new CharByteHashMap(keys.length);
