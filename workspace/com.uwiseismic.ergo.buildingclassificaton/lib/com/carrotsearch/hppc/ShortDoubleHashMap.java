             ...
             Cloneable
{
   ...
   *          expansion (inclusive).
   */
  public ShortDoubleHashMap(int expectedElements) {
   ...
   */
  public ShortDoubleHashMap(int expectedElements, double loadFactor) {
   ...
   *          consequences.
   */
  public ShortDoubleHashMap(int expectedElements, double loadFactor, HashOrderMixingStrategy orderMixer) {
   ...
   */
  public ShortDoubleHashMap(ShortDoubleAssociativeContainer container) {
    this(container.size());
    putAll(container);
  }
   ...
   */
  @Override
  public double addTo(short key, double incrementValue)
  {
   ...
   */
  protected boolean equalElements(ShortDoubleHashMap other) {
    if (other.size() != size()) {
      return false;
    }

    for (ShortDoubleCursor c : other) {
      short key =  c.key;
      if (!containsKey(key) ||
          !(Double.doubleToLongBits(get(key)) == Double.doubleToLongBits(c.value))) {
        return false;
      }
    }
  ...
  public final class KeysContainer extends AbstractShortCollection 
    ...
    private final ShortDoubleHashMap owner = ShortDoubleHashMap.this;
    ...
    public <T extends ShortProcedure> T forEach(final T procedure) {
      owner.forEach(new ShortDoubleProcedure() {
        @Override
    ...
    public <T extends ShortPredicate> T forEach(final T predicate) {
      owner.forEach(new ShortDoublePredicate() {
        @Override
  ...
  private final class ValuesContainer extends AbstractDoubleCollection {
    ...
    public int removeAll(final double e) {
      return owner.removeAll(new ShortDoublePredicate() {
        @Override
        public boolean apply(short key, double value) {
  ...
  public ShortDoubleHashMap clone() {
    try {
      /*  */
      ShortDoubleHashMap cloned = (ShortDoubleHashMap) super.clone();
   ...
   */
  public static  ShortDoubleHashMap from(short[] keys, double[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    ShortDoubleHashMap map = new ShortDoubleHashMap(keys.length);
   ...
   */
  protected void rehash(short[] fromKeys, double[] fromValues) {
   ...
   */
  protected void allocateThenInsertThenRehash(int slot, short pendingKey, double pendingValue) {
