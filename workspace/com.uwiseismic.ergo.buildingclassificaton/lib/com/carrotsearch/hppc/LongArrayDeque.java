...
/**
 * An array-backed {@link LongDeque}.
 */
...
public class LongArrayDeque 
  ...
  public 
        ...
        buffer = LongArrayList.EMPTY_ARRAY;
  ...
  public void clear() {
    if (head < tail) {
      Arrays.fill(buffer, head, tail, 0L);
    } else {
      Arrays.fill(buffer, 0, tail, 0L);
      Arrays.fill(buffer, head, buffer.length, 0L);
    }
    this.head = tail = 0;
  }
   ...
   */
  private void descendingForEach(LongProcedure procedure, int fromIndex, final int toIndex) {
   ...
   */
  private void descendingForEach(LongPredicate predicate, int fromIndex, final int toIndex) {
    if (fromIndex == toIndex)
      return;
  ...
  /**
   * Compare order-aligned elements against another {@link LongDeque}.
   */
