             ...
             Cloneable
{
   ...
   *          expansion (inclusive).
   */
  public ShortShortHashMap(int expectedElements) {
   ...
   */
  public ShortShortHashMap(int expectedElements, double loadFactor) {
   ...
   *          consequences.
   */
  public ShortShortHashMap(int expectedElements, double loadFactor, HashOrderMixingStrategy orderMixer) {
   ...
   */
  public ShortShortHashMap(ShortShortAssociativeContainer container) {
    this(container.size());
    putAll(container);
  }
   ...
   */
  @Override
  public short addTo(short key, short incrementValue)
  {
   ...
   */
  protected boolean equalElements(ShortShortHashMap other) {
    if (other.size() != size()) {
      return false;
    }

    for (ShortShortCursor c : other) {
      short key =  c.key;
      if (!containsKey(key) ||
          !((get(key)) == (c.value))) {
        return false;
  ...
  public final class KeysContainer extends AbstractShortCollection 
    ...
    private final ShortShortHashMap owner = ShortShortHashMap.this;
    ...
    public <T extends ShortProcedure> T forEach(final T procedure) {
      owner.forEach(new ShortShortProcedure() {
        @Override
    ...
    public <T extends ShortPredicate> T forEach(final T predicate) {
      owner.forEach(new ShortShortPredicate() {
        @Override
  ...
  private final class ValuesContainer extends AbstractShortCollection {
    ...
    @Override
    public int removeAll(final short e) {
      return owner.removeAll(new ShortShortPredicate() {
        @Override
        public boolean apply(short key, short value) {
          return ((value) == (e));
        }
  ...
  public ShortShortHashMap clone() {
    try {
      /*  */
      ShortShortHashMap cloned = (ShortShortHashMap) super.clone();
   ...
   */
  public static  ShortShortHashMap from(short[] keys, short[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    ShortShortHashMap map = new ShortShortHashMap(keys.length);
   ...
   */
  protected void rehash(short[] fromKeys, short[] fromValues) {
   ...
   */
  protected void allocateThenInsertThenRehash(int slot, short pendingKey, short pendingValue) {
