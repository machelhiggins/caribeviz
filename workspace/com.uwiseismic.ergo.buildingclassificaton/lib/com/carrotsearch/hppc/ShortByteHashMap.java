             ...
             Cloneable
{
   ...
   *          expansion (inclusive).
   */
  public ShortByteHashMap(int expectedElements) {
   ...
   */
  public ShortByteHashMap(int expectedElements, double loadFactor) {
   ...
   *          consequences.
   */
  public ShortByteHashMap(int expectedElements, double loadFactor, HashOrderMixingStrategy orderMixer) {
   ...
   */
  protected boolean equalElements(ShortByteHashMap other) {
    if (other.size() != size()) {
      return false;
    }

    for (ShortByteCursor c : other) {
      short key =  c.key;
      if (!containsKey(key) ||
          !((get(key)) == (c.value))) {
        return false;
  ...
  public final class KeysContainer extends AbstractShortCollection 
    ...
    private final ShortByteHashMap owner = ShortByteHashMap.this;
    ...
    public <T extends ShortProcedure> T forEach(final T procedure) {
      owner.forEach(new ShortByteProcedure() {
        @Override
    ...
    public <T extends ShortPredicate> T forEach(final T predicate) {
      owner.forEach(new ShortBytePredicate() {
        @Override
  ...
  private final class ValuesContainer extends AbstractByteCollection {
    ...
    public int removeAll(final byte e) {
      return owner.removeAll(new ShortBytePredicate() {
        @Override
        public boolean apply(short key, byte value) {
          return ((value) == (e));
        }
  ...
  public ShortByteHashMap clone() {
    try {
      /*  */
      ShortByteHashMap cloned = (ShortByteHashMap) super.clone();
   ...
   */
  public static  ShortByteHashMap from(short[] keys, byte[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    ShortByteHashMap map = new ShortByteHashMap(keys.length);
   ...
   */
  protected void rehash(short[] fromKeys, byte[] fromValues) {
   ...
   */
  protected void allocateThenInsertThenRehash(int slot, short pendingKey, byte pendingValue) {
