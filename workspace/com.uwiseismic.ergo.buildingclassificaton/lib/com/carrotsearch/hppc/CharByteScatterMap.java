...
/**
 * Same as {@link CharByteHashMap} but does not implement per-instance
...
public class CharByteScatterMap extends CharByteHashMap
{
   ...
   */
  public static  CharByteScatterMap from(char[] keys, byte[] values) {
    if (keys.length != values.length) {
      throw new IllegalArgumentException("Arrays of keys and values must have an identical length.");
    }

    CharByteScatterMap map = new CharByteScatterMap(keys.length);
