...
public class City extends AbstractLocation
{
    ...
    {
        return new Property( TAG_SELF, cityCode, StVincent.ID, "location" ); //$NON-NLS-2$ //$NON-NLS-3$
    }

    private void fromProperty( Property p )
    {
        cityCode = p.getValue();
        City s = StvincentUtils.getCityFromCodeString( cityCode );
        name = s.getName();
