...
import com.uwiseismic.gis.location.stlucia.utils.LocationUtils2;
...
public class StluciaLocationFactory implements LocationFactory
{
    private final static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger( StluciaLocationFactory.class );
    ...
    public final static Stlucia country = new Stlucia();
    ...
    {
        if ( l.getType().equals( Stlucia.TAG_SELF ) )
            return StluciaUtils.getCities();
        if ( l.getType().equals( City.TAG_SELF ) )
            return StluciaUtils.getDistrictsInCity( (City) l );
        if ( l.getType().equals( Region.TAG_SELF ) )
            return StluciaUtils.getAllElectoralDistricts( (Region) l );
    ...
    {
        return StluciaUtils.createLocationFromProperty( p );
    }
