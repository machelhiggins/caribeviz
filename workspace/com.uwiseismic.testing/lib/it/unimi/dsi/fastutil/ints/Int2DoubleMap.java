...
public interface Int2DoubleMap extends Int2DoubleFunction, Map<Integer, Double> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Integer, ? super Double> consumer) {
		final ObjectSet<Int2DoubleMap.Entry> entrySet = int2DoubleEntrySet();
		final Consumer<Int2DoubleMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Integer.valueOf(entry.getIntKey()), Double.valueOf(entry.getDoubleValue()));
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final int key, final Int2DoubleFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final double v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default double mergeDouble(final int key, final double value, final java.util.function.DoubleBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final double oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.5.1
	 */
	default double mergeDouble(final int key, final double value, final it.unimi.dsi.fastutil.doubles.DoubleBinaryOperator remappingFunction) {
