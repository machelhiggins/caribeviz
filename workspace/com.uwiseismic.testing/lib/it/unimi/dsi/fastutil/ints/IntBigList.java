...
public interface IntBigList extends BigList<Integer>, IntCollection, Comparable<BigList<? extends Integer>> {
	...
	default IntSpliterator spliterator() {
		return IntSpliterators.asSpliterator(iterator(), size64(), IntSpliterators.LIST_SPLITERATOR_CHARACTERISTICS);
	}
	...
	default void getElements(long from, int a[], int offset, int length) {
		getElements(from, new int[][] { a }, offset, length);
	}
	 ...
	 */
	default boolean addAll(final long index, final IntBigList l) {
	 ...
	 */
	default boolean addAll(final IntBigList l) {
	 ...
	 * @see BigList#addAll(long,Collection)
	 */
	default boolean addAll(final long index, final IntList l) {
		return addAll(index, (IntCollection)l);
	}
