...
public class IntMappedBigList extends AbstractIntBigList {
	...
	public static int LOG2_BYTES = Long.SIZE - 1 - Long.numberOfLeadingZeros(Integer.BYTES);
	/** @deprecated Use {@link #LOG2_BYTES}. */
	@Deprecated
	public static int LOG2_BITS = Long.SIZE - 1 - Long.numberOfLeadingZeros(Integer.BYTES);
	 ...
	 */
	public static IntMappedBigList map(final FileChannel fileChannel, final ByteOrder byteOrder, final MapMode mapMode) throws IOException {
		final long size = fileChannel.size() / Integer.BYTES;
		final int chunks = (int)((size + (CHUNK_SIZE - 1)) / CHUNK_SIZE);
		final IntBuffer[] buffer = new IntBuffer[chunks];
		for (int i = 0; i < chunks; i++) buffer[i] = fileChannel.map(mapMode, i * CHUNK_SIZE * Integer.BYTES, Math.min(CHUNK_SIZE, size - i * CHUNK_SIZE) * Integer.BYTES).order(byteOrder).asIntBuffer();
	...
	public IntMappedBigList copy() {
		return new IntMappedBigList(buffer.clone(), size, new boolean[n]);
	}
	...
	public int set(final long index, final int value) {
		final IntBuffer b = IntBuffer((int)(index >>> CHUNK_SHIFT));
