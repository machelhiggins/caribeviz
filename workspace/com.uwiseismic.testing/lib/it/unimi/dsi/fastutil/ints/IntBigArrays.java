...
public final class IntBigArrays {
	...
	public static AtomicIntegerArray[] newBigAtomicArray(final long length) {
		...
		final int residual = (int)(length & SEGMENT_MASK);
		if (residual != 0) {
			for (int i = 0; i < baseLength - 1; i++) base[i] = new AtomicIntegerArray(SEGMENT_SIZE);
			base[baseLength - 1] = new AtomicIntegerArray(residual);
		} else for (int i = 0; i < baseLength; i++) base[i] = new AtomicIntegerArray(SEGMENT_SIZE);
		return base;
	}
	...
	private static void insertionSortIndirect(final long[][] perm, final int[][] a, final int[][] b, final long from, final long to) {
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final long[][] perm, final int[][] a, final int[][] b, final long from, final long to, final boolean stable) {
		...
		while (stackPos > 0) {
			...
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This is the shift
																									...
																									// key
			// Count keys.
			for (long i = first + length; i-- != first;) count[((BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)]++;
			...
			if (stable) {
				for (long i = first + length; i-- != first;) BigArrays.set(support, --pos[((BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)], BigArrays.get(perm, i));
