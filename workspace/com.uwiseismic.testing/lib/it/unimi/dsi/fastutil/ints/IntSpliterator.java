...
public interface IntSpliterator extends Spliterator.OfInt {
	...
	default boolean tryAdvance(final Consumer<? super Integer> action) {
		...
		return tryAdvance(action instanceof java.util.function.IntConsumer ? (java.util.function.IntConsumer)action : (java.util.function.IntConsumer)action::accept);
	}
	 ...
	 */
	default boolean tryAdvance(final IntConsumer action) {
	...
	default long skip(final long n) {
		...
		long i = n;
		while (i-- != 0 && tryAdvance((int unused) -> {
		})) {
