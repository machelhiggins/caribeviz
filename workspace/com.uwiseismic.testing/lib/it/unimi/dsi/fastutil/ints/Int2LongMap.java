...
public interface Int2LongMap extends Int2LongFunction, Map<Integer, Long> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Integer, ? super Long> consumer) {
		final ObjectSet<Int2LongMap.Entry> entrySet = int2LongEntrySet();
		final Consumer<Int2LongMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Integer.valueOf(entry.getIntKey()), Long.valueOf(entry.getLongValue()));
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final int key, final Int2LongFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final long v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default long mergeLong(final int key, final long value, final java.util.function.LongBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.5.1
	 */
	default long mergeLong(final int key, final long value, final it.unimi.dsi.fastutil.longs.LongBinaryOperator remappingFunction) {
