...
public class IntArrayList extends AbstractIntList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 *          {@code Collector} equivalent in the Java API.
	 */
	public static IntArrayList toListWithExpectedSize(java.util.stream.IntStream stream, int expectedSize) {
		...
		return stream.collect(new IntCollections.SizeDecreasingSupplier<IntArrayList>(expectedSize, (int size) -> size <= DEFAULT_INITIAL_CAPACITY ? new IntArrayList() : new IntArrayList(size)), IntArrayList::add, IntArrayList::addAll);
	}
	...
	private void grow(int capacity) {
		...
		if (a != IntArrays.DEFAULT_EMPTY_ARRAY) capacity = (int)Math.max(Math.min((long)a.length + (a.length >> 1), it.unimi.dsi.fastutil.Arrays.MAX_ARRAY_SIZE), capacity);
