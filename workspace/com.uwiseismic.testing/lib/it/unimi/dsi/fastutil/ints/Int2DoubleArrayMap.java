	...
	private final class EntrySet extends AbstractObjectSet<Int2DoubleMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Int2DoubleMap.Entry> iterator() {
			return new ObjectIterator<Int2DoubleMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Int2DoubleMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractInt2DoubleMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Int2DoubleMap.Entry> implements ObjectSpliterator<Int2DoubleMap.Entry> {
			...
			protected final Int2DoubleMap.Entry get(int location) {
				return new AbstractInt2DoubleMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Int2DoubleMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractInt2DoubleMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Int2DoubleMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
