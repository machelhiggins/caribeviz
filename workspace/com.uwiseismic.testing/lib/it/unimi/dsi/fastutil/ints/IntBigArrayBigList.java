...
public class IntBigArrayBigList extends AbstractIntBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public IntBigArrayBigList(final IntCollection c) {
		...
		if (c instanceof IntBigList) {
			((IntBigList)c).getElements(0, a, 0, size = Size64.sizeOf(c));
		} else {
	...
	public static IntBigArrayBigList of(final int... init) {
	 ...
	 */
	public static IntBigArrayBigList toBigList(java.util.stream.IntStream stream) {
		return stream.collect(IntBigArrayBigList::new, IntBigArrayBigList::add, IntBigArrayBigList::addAll);
	}
	 ...
	 */
	public static IntBigArrayBigList toBigListWithExpectedSize(java.util.stream.IntStream stream, long expectedSize) {
		return stream.collect(() -> new IntBigArrayBigList(expectedSize), IntBigArrayBigList::add, IntBigArrayBigList::addAll);
	}
	...
	public void ensureCapacity(final long capacity) {
		if (capacity <= length(a) || a == IntBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;
	...
	@Override
	public boolean addAll(long index, final IntCollection c) {
		...
		size += n;
		assert size <= length(a);
		while (n-- != 0) BigArrays.set(a, index++, i.nextInt());
		return true;
	}

	@Override
	public boolean addAll(final long index, final IntBigList list) {
		ensureIndex(index);
	...
	@Override
	public boolean addAll(final long index, final IntList list) {
		ensureIndex(index);
		int n = list.size();
		if (n == 0) return false;
		grow(size + n);
	...
	private class SubList extends AbstractIntBigList.IntRandomAccessSubList {
		...
		public boolean equals(Object o) {
			...
			if (o instanceof IntBigArrayBigList.SubList) {
		...
		public int compareTo(final BigList<? extends Integer> l) {
			...
			if (l instanceof IntBigArrayBigList.SubList) {
	 ...
	 */
	@Override
	public void getElements(final long from, final int[] a, final int offset, final int length) {
	 ...
	 */
	@Override
	public void setElements(final long index, final int[][] a, final long offset, final long length) {
		BigArrays.copy(a, offset, this.a, index, length);
	}
	...
	private final class Spliterator implements IntSpliterator {
		...
		public Spliterator() {
			this(0, IntBigArrayBigList.this.size, false);
		}
		...
		private long getWorkingMax() {
			return hasSplit ? max : IntBigArrayBigList.this.size;
		}
	...
	public IntBigArrayBigList clone() {
		...
		if (getClass() == IntBigArrayBigList.class) {
			...
			c.a = IntBigArrays.newBigArray(size);
		}
	...
	public boolean equals(final Object o) {
		...
		if (o instanceof IntBigArrayBigList.SubList) {
			...
			return ((IntBigArrayBigList.SubList)o).equals(this);
		}
		return super.equals(o);
	}
	...
	public int compareTo(final BigList<? extends Integer> l) {
		...
		if (l instanceof IntBigArrayBigList.SubList) {
			...
			return -((IntBigArrayBigList.SubList)l).compareTo(this);
		}
		return super.compareTo(l);
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		for (int i = 0; i < size; i++) s.writeInt(BigArrays.get(a, i));
	}

	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		s.defaultReadObject();
		a = IntBigArrays.newBigArray(size);
		for (int i = 0; i < size; i++) BigArrays.set(a, i, s.readInt());
	}
}
