...
public class ShortMappedBigList extends AbstractShortBigList {
	...
	public static int LOG2_BYTES = Long.SIZE - 1 - Long.numberOfLeadingZeros(Short.BYTES);
	/** @deprecated Use {@link #LOG2_BYTES}. */
	@Deprecated
	public static int LOG2_BITS = Long.SIZE - 1 - Long.numberOfLeadingZeros(Short.BYTES);
	 ...
	 */
	public static ShortMappedBigList map(final FileChannel fileChannel, final ByteOrder byteOrder, final MapMode mapMode) throws IOException {
		final long size = fileChannel.size() / Short.BYTES;
		final int chunks = (int)((size + (CHUNK_SIZE - 1)) / CHUNK_SIZE);
		final ShortBuffer[] buffer = new ShortBuffer[chunks];
		for (int i = 0; i < chunks; i++) buffer[i] = fileChannel.map(mapMode, i * CHUNK_SIZE * Short.BYTES, Math.min(CHUNK_SIZE, size - i * CHUNK_SIZE) * Short.BYTES).order(byteOrder).asShortBuffer();
	...
	public ShortMappedBigList copy() {
		return new ShortMappedBigList(buffer.clone(), size, new boolean[n]);
	}
	...
	public short set(final long index, final short value) {
		final ShortBuffer b = ShortBuffer((int)(index >>> CHUNK_SHIFT));
