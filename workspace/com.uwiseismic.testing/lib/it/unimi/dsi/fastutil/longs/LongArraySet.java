...
public class LongArraySet extends AbstractLongSet implements java.io.Serializable, Cloneable {
	 ...
	 * @param c a collection.
	 */
	public LongArraySet(final Set<? extends Long> c) {
		this(c.size());
		int i = 0;
		for (Long x : c) {
			a[i] = (x).longValue();
	 ...
	 */
	public static LongArraySet of(final long e) {
	...
	public static LongArraySet of(final long... a) {
		if (a.length == 2) {
			if (((a[0]) == (a[1]))) {
	...
	public static LongArraySet ofUnchecked(final long... a) {
	...
	private final class Spliterator implements LongSpliterator {
		...
		private int getWorkingMax() {
			return hasSplit ? max : LongArraySet.this.size;
		}
