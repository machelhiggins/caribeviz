...
public interface Long2LongMap extends Long2LongFunction, Map<Long, Long> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Long, ? super Long> consumer) {
		final ObjectSet<Long2LongMap.Entry> entrySet = long2LongEntrySet();
		final Consumer<Long2LongMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Long.valueOf(entry.getLongKey()), Long.valueOf(entry.getLongValue()));
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final long key, final Long2LongFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final long v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default long mergeLong(final long key, final long value, final java.util.function.LongBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.5.1
	 */
	default long mergeLong(final long key, final long value, final it.unimi.dsi.fastutil.longs.LongBinaryOperator remappingFunction) {
