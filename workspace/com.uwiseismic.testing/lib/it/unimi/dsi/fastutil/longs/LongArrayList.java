...
public class LongArrayList extends AbstractLongList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 *          {@code Collector} equivalent in the Java API.
	 */
	public static LongArrayList toListWithExpectedSize(java.util.stream.LongStream stream, int expectedSize) {
		...
		return stream.collect(new LongCollections.SizeDecreasingSupplier<LongArrayList>(expectedSize, (int size) -> size <= DEFAULT_INITIAL_CAPACITY ? new LongArrayList() : new LongArrayList(size)), LongArrayList::add, LongArrayList::addAll);
	}
	...
	private void grow(int capacity) {
		...
		if (a != LongArrays.DEFAULT_EMPTY_ARRAY) capacity = (int)Math.max(Math.min((long)a.length + (a.length >> 1), it.unimi.dsi.fastutil.Arrays.MAX_ARRAY_SIZE), capacity);
