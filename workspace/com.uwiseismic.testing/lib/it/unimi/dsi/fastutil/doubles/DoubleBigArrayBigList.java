...
public class DoubleBigArrayBigList extends AbstractDoubleBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public DoubleBigArrayBigList(final DoubleCollection c) {
		...
		if (c instanceof DoubleBigList) {
			((DoubleBigList)c).getElements(0, a, 0, size = Size64.sizeOf(c));
		} else {
	...
	public static DoubleBigArrayBigList of(final double... init) {
	...
	public static DoubleBigArrayBigList toBigList(java.util.stream.DoubleStream stream) {
		return stream.collect(DoubleBigArrayBigList::new, DoubleBigArrayBigList::add, DoubleBigArrayBigList::addAll);
	}
	 ...
	 */
	public static DoubleBigArrayBigList toBigListWithExpectedSize(java.util.stream.DoubleStream stream, long expectedSize) {
		return stream.collect(() -> new DoubleBigArrayBigList(expectedSize), DoubleBigArrayBigList::add, DoubleBigArrayBigList::addAll);
	}
	...
	public void ensureCapacity(final long capacity) {
		if (capacity <= length(a) || a == DoubleBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;
	...
	public long indexOf(final double k) {
		for (long i = 0; i < size; i++) if ((Double.doubleToLongBits(k) == Double.doubleToLongBits(BigArrays.get(a, i)))) return i;
		return -1;
	}

	@Override
	public long lastIndexOf(final double k) {
		for (long i = size; i-- != 0;) if ((Double.doubleToLongBits(k) == Double.doubleToLongBits(BigArrays.get(a, i)))) return i;
		return -1;
	}
	...
	@Override
	public boolean addAll(long index, final DoubleCollection c) {
		...
		size += n;
		assert size <= length(a);
		while (n-- != 0) BigArrays.set(a, index++, i.nextDouble());
		return true;
	}

	@Override
	public boolean addAll(final long index, final DoubleBigList list) {
		ensureIndex(index);
	...
	@Override
	public boolean addAll(final long index, final DoubleList list) {
		ensureIndex(index);
		int n = list.size();
		if (n == 0) return false;
		grow(size + n);
	...
	private class SubList extends AbstractDoubleBigList.DoubleRandomAccessSubList {
		...
		public boolean equals(Object o) {
			...
			if (o instanceof DoubleBigArrayBigList.SubList) {
		...
		public int compareTo(final BigList<? extends Double> l) {
			...
			if (l instanceof DoubleBigArrayBigList.SubList) {
	 ...
	 */
	@Override
	public void getElements(final long from, final double[] a, final int offset, final int length) {
	 ...
	 */
	@Override
	public void setElements(final long index, final double[][] a, final long offset, final long length) {
		BigArrays.copy(a, offset, this.a, index, length);
	}
	...
	private final class Spliterator implements DoubleSpliterator {
		...
		public Spliterator() {
			this(0, DoubleBigArrayBigList.this.size, false);
		}
		...
		private long getWorkingMax() {
			return hasSplit ? max : DoubleBigArrayBigList.this.size;
		}
	...
	public DoubleBigArrayBigList clone() {
		...
		if (getClass() == DoubleBigArrayBigList.class) {
			c = new DoubleBigArrayBigList(size);
			c.size = size;
		} else {
			try {
			...
			c.a = DoubleBigArrays.newBigArray(size);
		}
	...
	public boolean equals(final Object o) {
		...
		if (o instanceof DoubleBigArrayBigList.SubList) {
			...
			return ((DoubleBigArrayBigList.SubList)o).equals(this);
		}
		return super.equals(o);
	}
	...
	public int compareTo(final BigList<? extends Double> l) {
		...
		if (l instanceof DoubleBigArrayBigList.SubList) {
			...
			return -((DoubleBigArrayBigList.SubList)l).compareTo(this);
		}
		return super.compareTo(l);
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		for (int i = 0; i < size; i++) s.writeDouble(BigArrays.get(a, i));
	}

	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		s.defaultReadObject();
		a = DoubleBigArrays.newBigArray(size);
		for (int i = 0; i < size; i++) BigArrays.set(a, i, s.readDouble());
	}
}
