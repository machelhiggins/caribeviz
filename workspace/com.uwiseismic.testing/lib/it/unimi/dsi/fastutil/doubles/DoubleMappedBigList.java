...
public class DoubleMappedBigList extends AbstractDoubleBigList {
	...
	public static int LOG2_BYTES = Long.SIZE - 1 - Long.numberOfLeadingZeros(Double.BYTES);
	/** @deprecated Use {@link #LOG2_BYTES}. */
	@Deprecated
	public static int LOG2_BITS = Long.SIZE - 1 - Long.numberOfLeadingZeros(Double.BYTES);
	 ...
	 *            list.
	 */
	protected DoubleMappedBigList(final DoubleBuffer[] buffer, final long size, final boolean[] readyToUse) {
		this.buffer = buffer;
	 ...
	 */
	public static DoubleMappedBigList map(final FileChannel fileChannel) throws IOException {
	 ...
	 * @see #map(FileChannel, ByteOrder, MapMode)
	 */
	public static DoubleMappedBigList map(final FileChannel fileChannel, final ByteOrder byteOrder) throws IOException {
	 ...
	 */
	public static DoubleMappedBigList map(final FileChannel fileChannel, final ByteOrder byteOrder, final MapMode mapMode) throws IOException {
		final long size = fileChannel.size() / Double.BYTES;
		final int chunks = (int)((size + (CHUNK_SIZE - 1)) / CHUNK_SIZE);
		final DoubleBuffer[] buffer = new DoubleBuffer[chunks];
		for (int i = 0; i < chunks; i++) buffer[i] = fileChannel.map(mapMode, i * CHUNK_SIZE * Double.BYTES, Math.min(CHUNK_SIZE, size - i * CHUNK_SIZE) * Double.BYTES).order(byteOrder).asDoubleBuffer();
	...
	private DoubleBuffer DoubleBuffer(final int n) {
	...
	public DoubleMappedBigList copy() {
		return new DoubleMappedBigList(buffer.clone(), size, new boolean[n]);
	}
	...
	@Override
	public double set(final long index, final double value) {
		final DoubleBuffer b = DoubleBuffer((int)(index >>> CHUNK_SHIFT));
		final int i = (int)(index & CHUNK_MASK);
		final double previousValue = b.get(i);
		b.put(i, value);
