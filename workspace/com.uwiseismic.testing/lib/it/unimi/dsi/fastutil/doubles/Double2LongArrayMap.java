	...
	private final class EntrySet extends AbstractObjectSet<Double2LongMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Double2LongMap.Entry> iterator() {
			return new ObjectIterator<Double2LongMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Double2LongMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractDouble2LongMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Double2LongMap.Entry> implements ObjectSpliterator<Double2LongMap.Entry> {
			...
			protected final Double2LongMap.Entry get(int location) {
				return new AbstractDouble2LongMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Double2LongMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractDouble2LongMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Double2LongMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
