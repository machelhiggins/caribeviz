...
public class DoubleOpenHashSet extends AbstractDoubleSet implements java.io.Serializable, Cloneable, Hash {
	...
	public static DoubleOpenHashSet toSetWithExpectedSize(java.util.stream.DoubleStream stream, int expectedSize) {
		...
		return stream.collect(new DoubleCollections.SizeDecreasingSupplier<DoubleOpenHashSet>(expectedSize, (int size) -> size <= Hash.DEFAULT_INITIAL_SIZE ? new DoubleOpenHashSet() : new DoubleOpenHashSet(size)), DoubleOpenHashSet::add, DoubleOpenHashSet::addAll);
	}
