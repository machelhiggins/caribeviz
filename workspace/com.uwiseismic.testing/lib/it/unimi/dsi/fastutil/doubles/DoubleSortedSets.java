...
public final class DoubleSortedSets {
	...
	public static class Singleton extends DoubleSets.Singleton implements DoubleSortedSet, java.io.Serializable, Cloneable {
		...
		Singleton(final double element) {
			this(element, null);
		}
		...
		public DoubleSpliterator spliterator() {
			return DoubleSpliterators.singleton(element, comparator);
		}

		@Override

		public DoubleSortedSet subSet(final double from, final double to) {
			if (compare(from, element) <= 0 && compare(element, to) < 0) return this;
			return EMPTY_SET;
		}
		...
		public DoubleSortedSet headSet(final double to) {
			if (compare(element, to) < 0) return this;
			return EMPTY_SET;
		}
		...
		public DoubleSortedSet tailSet(final double from) {
			if (compare(from, element) <= 0) return this;
			return EMPTY_SET;
		}
	...
	/**
	 * Returns a type-specific immutable sorted set containing only the specified element. The returned
	 * sorted set is serializable and cloneable.
	 *
	...
	/**
	 * Returns a type-specific immutable sorted set containing only the specified element, and using a
	 * specified comparator. The returned sorted set is serializable and cloneable.
	 *
	 * @param element the only element of the returned sorted set.
