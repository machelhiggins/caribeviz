	...
	public boolean add(final double k) {
		...
		if ((strategy.equals((k), (0)))) {
			...
			if (!(Double.doubleToLongBits(curr = key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(k))) & mask]) == 0)) {
				...
				while (!(Double.doubleToLongBits(curr = key[pos = (pos + 1) & mask]) == 0)) if ((strategy.equals((curr), (k)))) return false;
			}
			key[pos] = k;
		}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = size, pos; i-- != 0;) {
			...
			if ((strategy.equals((k), (0)))) {
				...
				if (!(Double.doubleToLongBits(key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(k))) & mask]) == 0)) while (!(Double.doubleToLongBits(key[pos = (pos + 1) & mask]) == 0));
			}
			key[pos] = k;
		}
		if (ASSERTS) checkTable();
