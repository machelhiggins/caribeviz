	...
	private final class EntrySet extends AbstractObjectSet<Double2IntMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Double2IntMap.Entry> iterator() {
			return new ObjectIterator<Double2IntMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Double2IntMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractDouble2IntMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Double2IntMap.Entry> implements ObjectSpliterator<Double2IntMap.Entry> {
			...
			protected final Double2IntMap.Entry get(int location) {
				return new AbstractDouble2IntMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Double2IntMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractDouble2IntMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Double2IntMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
