 ...
 * of lines of text.
 *
 * <h2>Parallel operations</h2> Some algorithms provide a parallel version that will by default use
...
public final class DoubleBigArrays {
	...
	private static void selectionSort(final double[][] a, final long from, final long to) {
		for (long i = from; i < to - 1; i++) {
			long m = i;
			for (long j = i + 1; j < to; j++) if ((Double.compare((BigArrays.get(a, j)), (BigArrays.get(a, m))) < 0)) m = j;
			if (m != i) BigArrays.swap(a, i, m);
		}
	}
	 ...
	 * @param comp the comparator to determine the sorting order.
	 */
	public static void parallelQuickSort(final double[][] x, final long from, final long to, final DoubleComparator comp) {
		ForkJoinPool pool = getPool();
	 ...
	 *         <code>(-(<i>insertion point</i>) - 1)</code>. The <i>insertion point</i> is defined as
	 ...
	 * 5&minus;27 (1993), and further improved using the digit-oracle idea described by Juha
	 * K&auml;rkk&auml;inen and Tommi Rantala in &ldquo;Engineering radix sort for strings&rdquo;,
	...
	public static void radixSort(final double[][] a, final long from, final long to) {
		...
		while (offsetPos > 0) {
			...
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This is the shift
																									...
																									// key
			// Count keys.
			for (long i = length; i-- != 0;) BigArrays.set(digit, i, (byte)(((fixDouble(BigArrays.get(a, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
	...
	private static void selectionSort(final double[][] a, final double[][] b, final long from, final long to) {
		for (long i = from; i < to - 1; i++) {
			long m = i;
			for (long j = i + 1; j < to; j++) if ((Double.compare((BigArrays.get(a, j)), (BigArrays.get(a, m))) < 0) || (Double.compare((BigArrays.get(a, j)), (BigArrays.get(a, m))) == 0) && (Double.compare((BigArrays.get(b, j)), (BigArrays.get(b, m))) < 0)) m = j;
			if (m != i) {
	...
	public static void radixSort(final double[][] a, final double[][] b, final long from, final long to) {
		...
		while (offsetPos > 0) {
			...
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This is the shift
																									...
																									// key
			// Count keys.
			for (long i = length; i-- != 0;) BigArrays.set(digit, i, (byte)(((fixDouble(BigArrays.get(k, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
			for (long i = length; i-- != 0;) count[BigArrays.get(digit, i) & 0xFF]++;
			// Compute cumulative distribution and push non-singleton keys on stack.
			int lastUsed = -1;
			long p = 0;
	...
	private static void insertionSortIndirect(final long[][] perm, final double[][] a, final double[][] b, final long from, final long to) {
		for (long i = from; ++i < to;) {
			long t = BigArrays.get(perm, i);
			long j = i;
			for (long u = BigArrays.get(perm, j - 1); (Double.compare((BigArrays.get(a, t)), (BigArrays.get(a, u))) < 0) || (Double.compare((BigArrays.get(a, t)), (BigArrays.get(a, u))) == 0) && (Double.compare((BigArrays.get(b, t)), (BigArrays.get(b, u))) < 0); u = BigArrays.get(perm, --j - 1)) {
	 ...
	 */
	public static void radixSortIndirect(final long[][] perm, final double[][] a, final double[][] b, final boolean stable) {
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final long[][] perm, final double[][] a, final double[][] b, final long from, final long to, final boolean stable) {
		...
		while (stackPos > 0) {
			...
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This is the shift
																									...
																									// key
			// Count keys.
			for (long i = first + length; i-- != first;) count[(int)(fixDouble(BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)]++;
			...
			if (stable) {
				for (long i = first + length; i-- != first;) BigArrays.set(support, --pos[(int)(fixDouble(BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)], BigArrays.get(perm, i));
