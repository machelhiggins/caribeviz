...
public interface DoubleCollection extends Collection<Double>, DoubleIterable {
	 ...
	 * primitive types' {@code Collection}s that have use for widened iterators.
	 *
	...
	default DoubleSpliterator spliterator() {
		return DoubleSpliterators.asSpliterator(iterator(), sizeOf(this), DoubleSpliterators.COLLECTION_SPLITERATOR_CHARACTERISTICS);
	}
	 ...
	 * primitive types' {@code Collection}s that have use for widened spliterators.
	 *
	...
	default boolean removeIf(final java.util.function.Predicate<? super Double> filter) {
		return removeIf(filter instanceof java.util.function.DoublePredicate ? ((java.util.function.DoublePredicate)filter) : (java.util.function.DoublePredicate)key -> filter.test(Double.valueOf(key)));
	}
	 ...
	 * to disambiguate. Instead, override the {@code removeIf()} overload that uses the JDK's primitive
	...
	default boolean removeIf(final DoublePredicate filter) {
		return removeIf((java.util.function.DoublePredicate)filter);
	}
	...
	@Deprecated
	@Override
	default java.util.stream.Stream<Double> stream() {
	...
	@Deprecated
	@Override
	default java.util.stream.Stream<Double> parallelStream() {
