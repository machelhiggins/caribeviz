...
public class DoubleImmutableList extends DoubleLists.ImmutableListBase implements DoubleList, RandomAccess, Cloneable, java.io.Serializable {
	...
	public int indexOf(final double k) {
		for (int i = 0,
				size = a.length; i < size; i++) if ((Double.doubleToLongBits(k) == Double.doubleToLongBits(a[i]))) return i;
		return -1;
	}
	...
	public double[] toDoubleArray() {
		if (a.length == 0) return DoubleArrays.EMPTY_ARRAY;
		return a.clone();
	}
	...
	private final class Spliterator implements DoubleSpliterator {
		...
		public int characteristics() {
			return DoubleSpliterators.LIST_SPLITERATOR_CHARACTERISTICS | java.util.Spliterator.IMMUTABLE;
		}
