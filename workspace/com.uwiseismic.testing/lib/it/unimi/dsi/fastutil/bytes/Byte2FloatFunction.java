...
public interface Byte2FloatFunction extends Function<Byte, Float>, java.util.function.IntToDoubleFunction {
	...
	default <T> it.unimi.dsi.fastutil.bytes.Byte2ObjectFunction<T> andThenObject(it.unimi.dsi.fastutil.floats.Float2ObjectFunction<? extends T> after) {
		return k -> after.get(get(k));
	}

	default <T> it.unimi.dsi.fastutil.objects.Object2FloatFunction<T> composeObject(it.unimi.dsi.fastutil.objects.Object2ByteFunction<? super T> before) {
		return k -> get(before.getByte(k));
	}

	default <T> it.unimi.dsi.fastutil.bytes.Byte2ReferenceFunction<T> andThenReference(it.unimi.dsi.fastutil.floats.Float2ReferenceFunction<? extends T> after) {
		return k -> after.get(get(k));
	}

	default <T> it.unimi.dsi.fastutil.objects.Reference2FloatFunction<T> composeReference(it.unimi.dsi.fastutil.objects.Reference2ByteFunction<? super T> before) {
		return k -> get(before.getByte(k));
	}
}
