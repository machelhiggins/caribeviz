 ...
 * indices&mdash;all operations are performed directly on the array. The array is enlarged as
...
public class ObjectArrayIndirectPriorityQueue<K> implements IndirectPriorityQueue<K> {
	...
	private int findFirst() {
		...
		if (c == null) while (i-- != 0) {
			if ((((Comparable<K>)(refArray[array[i]])).compareTo(first) < 0)) first = refArray[array[firstIndex = i]];
		}
		else while (i-- != 0) {
	...
	private int findLast() {
		...
		if (c == null) {
			while (i-- != 0) if ((((Comparable<K>)(last)).compareTo(refArray[array[i]]) < 0)) last = refArray[array[lastIndex = i]];
		} else {
	 ...
	 *
	 * <p>
	 * Note that for efficiency reasons this method will <em>not</em> throw an exception when {@code x}
	...
	public void enqueue(int x) {
		...
		if (firstIndexValid) {
			if (c == null) {
				if ((((Comparable<K>)(refArray[x])).compareTo(refArray[array[firstIndex]]) < 0)) firstIndex = size;
	 ...
	 *
	 * <p>
	 * Note that for efficiency reasons this method will <em>not</em> throw an exception when
