 ...
 * is enlarged and shrunk as needed. You can use the {@link #trim()} method to reduce its memory
 * usage, if necessary.
 *
 * <p>
...
public class ObjectArrayFIFOQueue<K> implements PriorityQueue<K>, Serializable {
	...
	/**
	 * The end position in {@link #array}. It is always strictly smaller than {@link #length}. Might be
	 * actually smaller than {@link #start} because {@link #array} is used cyclically.
	 */
	...
	public ObjectArrayFIFOQueue(final int capacity) {
		...
		array = (K[])new Object[Math.max(1, capacity + 1)];
		length = array.length;
	}
