...
public final class ObjectSpliterators {
	 ...
	 */
	public static class EmptySpliterator<K> implements ObjectSpliterator<K>, java.io.Serializable, Cloneable {
	 ...
	 *
	 * <p>
	 * The class of the object returned represent an abstract empty spliterator that can iterate as a
	 * type-specific (list) spliterator.
	 *
	 * <p>
	...
	private static class ArraySpliterator<K> implements ObjectSpliterator<K> {
		...
		public ArraySpliterator(final K[] array, final int offset, final int length, int additionalCharacteristics) {
			this.array = array;
			this.offset = offset;
			this.length = length;
	...
	private static class ArraySpliteratorWithComparator<K> extends ArraySpliterator<K> {
		...
		public ArraySpliteratorWithComparator(final K[] array, final int offset, final int length, int additionalCharacteristics, final Comparator<? super K> comparator) {
	 ...
	 *         position {@code offset}.
	 */
	public static <K> ObjectSpliterator<K> wrapPreSorted(final K[] array, final int offset, final int length, final int additionalCharacteristics, Comparator<? super K> comparator) {
		ObjectArrays.ensureOffsetLength(array, offset, length);
	 ...
	 *         position {@code offset}.
	 */
	public static <K> ObjectSpliterator<K> wrapPreSorted(final K[] array, final int offset, final int length, Comparator<? super K> comparator) {
	 ...
	 *         position {@code offset}.
	 */
	public static <K> ObjectSpliterator<K> wrapPreSorted(final K[] array, Comparator<? super K> comparator) {
	...
	private static class SpliteratorWrapper<K> implements ObjectSpliterator<K> {
		...
		@Override
		public Comparator<? super K> getComparator() {
			return ObjectComparators.asObjectComparator(i.getComparator());
		}

		@Override
		public ObjectSpliterator<K> trySplit() {
			Spliterator<K> innerSplit = i.trySplit();
	...
	public static <K> ObjectSpliterator<K> asObjectSpliterator(final Spliterator<K> i) {
		if (i instanceof ObjectSpliterator) return (ObjectSpliterator<K>)i;
	...
	public static <K> ObjectSpliterator<K> asObjectSpliterator(final Spliterator<K> i, final Comparator<? super K> comparatorOverride) {
		if (i instanceof ObjectSpliterator) throw new IllegalArgumentException("Cannot override comparator on instance that is already a " + ObjectSpliterator.class.getSimpleName());
	...
	public static <K> void onEachMatching(final Spliterator<K> spliterator, final Predicate<? super K> predicate, final Consumer<? super K> action) {
		...
		Objects.requireNonNull(action);
		spliterator.forEachRemaining((K value) -> {
	...
	public static abstract class AbstractIndexBasedSpliterator<K> extends AbstractObjectSpliterator<K> {
		 ...
		 */
		protected abstract ObjectSpliterator<K> makeForSplit(int pos, int maxPos);
	...
	private static class SpliteratorConcatenator<K> implements ObjectSpliterator<K> {
		...
		public SpliteratorConcatenator(final ObjectSpliterator<? extends K> a[], int offset, int length) {
			this.a = a;
			this.offset = offset;
			this.length = length;
		...
		public Comparator<? super K> getComparator() {
			if (length == 1 && ((characteristics & Spliterator.SORTED) != 0)) {
				return (Comparator<? super K>)a[offset].getComparator();
			}
		...
		public ObjectSpliterator<K> trySplit() {
			...
			case 1: {
				...
				@SuppressWarnings("unchecked") // This is safe because spliterators only "give" K, never "take" them.
				ObjectSpliterator<K> split = (ObjectSpliterator<K>)a[offset].trySplit();
			...
			case 2: {
				...
				@SuppressWarnings("unchecked") // This is safe because spliterators only "give" K, never "take" them.
				ObjectSpliterator<K> split = (ObjectSpliterator<K>)a[offset++];
				--length;
				// assert length == 1;
	...
	private static class SpliteratorFromIterator<K> implements ObjectSpliterator<K> {
		...
		SpliteratorFromIterator(final ObjectIterator<? extends K> iter, int characteristics) {
			this.iter = iter;
			this.characteristics = BASE_SPLITERATOR_CHARACTERISTICS | characteristics;
			knownSize = false;
		}

		SpliteratorFromIterator(final ObjectIterator<? extends K> iter, long size, int additionalCharacteristics) {
			this.iter = iter;
			knownSize = true;
			this.size = size;
		...
		public long skip(long n) {
			if (n < 0) throw new IllegalArgumentException("Argument must be nonnegative: " + n);
			if (iter instanceof ObjectBigListIterator) {
				long skipped = ((ObjectBigListIterator<? extends K>)iter).skip(n);
				size -= skipped;
				return skipped;
			} else {
				long skippedSoFar = 0;
	...
	private static class SpliteratorFromIteratorWithComparator<K> extends SpliteratorFromIterator<K> {
		...
		SpliteratorFromIteratorWithComparator(final ObjectIterator<? extends K> iter, int additionalCharacteristics, final Comparator<? super K> comparator) {
			super(iter, additionalCharacteristics | SORTED_CHARACTERISTICS);
			this.comparator = comparator;
		}

		SpliteratorFromIteratorWithComparator(final ObjectIterator<? extends K> iter, long size, int additionalCharacteristics, final Comparator<? super K> comparator) {
