...
public interface Object2DoubleMap<K> extends Object2DoubleFunction<K>, Map<K, Double> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Double> consumer) {
		final ObjectSet<Object2DoubleMap.Entry<K>> entrySet = object2DoubleEntrySet();
		final Consumer<Object2DoubleMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Double.valueOf(entry.getDoubleValue()));
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final K key, final java.util.function.ToDoubleFunction<? super K> mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final K key, final Object2DoubleFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final double v = getDouble(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default double mergeDouble(final K key, final double value, final java.util.function.DoubleBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final double oldValue = getDouble(key), drv = defaultReturnValue();
