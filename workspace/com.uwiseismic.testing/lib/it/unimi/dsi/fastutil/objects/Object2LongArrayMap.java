...
public class Object2LongArrayMap<K> extends AbstractObject2LongMap<K> implements java.io.Serializable, Cloneable {
	...
	private final class EntrySet extends AbstractObjectSet<Object2LongMap.Entry<K>> implements FastEntrySet<K> {
		...
		public ObjectIterator<Object2LongMap.Entry<K>> iterator() {
			return new ObjectIterator<Object2LongMap.Entry<K>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Object2LongMap.Entry<K>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractObject2LongMap.BasicEntry<>((K)key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Object2LongMap.Entry<K>> implements ObjectSpliterator<Object2LongMap.Entry<K>> {
			...
			protected final Object2LongMap.Entry<K> get(int location) {
				return new AbstractObject2LongMap.BasicEntry<>((K)key[location], value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Object2LongMap.Entry<K>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractObject2LongMap.BasicEntry<>((K)key[i], value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Object2LongMap.Entry<K>> action) {
			final BasicEntry<K> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = (K)key[i];
	...
	private final class ValuesCollection extends AbstractLongCollection {
		...
		final class ValuesSpliterator extends it.unimi.dsi.fastutil.longs.LongSpliterators.EarlyBindingSizeIndexBasedSpliterator implements it.unimi.dsi.fastutil.longs.LongSpliterator {
			...
			@Override
			public int characteristics() {
				return it.unimi.dsi.fastutil.longs.LongSpliterators.COLLECTION_SPLITERATOR_CHARACTERISTICS | java.util.Spliterator.SUBSIZED | java.util.Spliterator.ORDERED;
			}
