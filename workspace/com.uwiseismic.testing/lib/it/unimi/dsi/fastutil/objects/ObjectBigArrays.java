 ...
 * <em>significantly slower</em> than using {@code new}. This phenomenon is particularly evident in
...
public final class ObjectBigArrays {
	...
	private static <K> void selectionSort(final K[][] a, final long from, final long to) {
		for (long i = from; i < to - 1; i++) {
			long m = i;
			for (long j = i + 1; j < to; j++) if ((((Comparable<K>)(BigArrays.get(a, j))).compareTo(BigArrays.get(a, m)) < 0)) m = j;
			if (m != i) BigArrays.swap(a, i, m);
		}
	}
