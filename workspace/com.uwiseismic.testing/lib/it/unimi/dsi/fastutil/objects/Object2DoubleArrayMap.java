	...
	private final class EntrySet extends AbstractObjectSet<Object2DoubleMap.Entry<K>> implements FastEntrySet<K> {
		...
		public ObjectIterator<Object2DoubleMap.Entry<K>> iterator() {
			return new ObjectIterator<Object2DoubleMap.Entry<K>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Object2DoubleMap.Entry<K>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractObject2DoubleMap.BasicEntry<>((K)key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Object2DoubleMap.Entry<K>> implements ObjectSpliterator<Object2DoubleMap.Entry<K>> {
			...
			protected final Object2DoubleMap.Entry<K> get(int location) {
				return new AbstractObject2DoubleMap.BasicEntry<>((K)key[location], value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Object2DoubleMap.Entry<K>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractObject2DoubleMap.BasicEntry<>((K)key[i], value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Object2DoubleMap.Entry<K>> action) {
			final BasicEntry<K> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = (K)key[i];
