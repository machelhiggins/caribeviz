...
public class ObjectOpenHashSet<K> extends AbstractObjectSet<K> implements java.io.Serializable, Cloneable, Hash {
	...
	public static <K> Collector<K, ?, ObjectOpenHashSet<K>> toSetWithExpectedSize(int expectedSize) {
		...
		return Collector.of(new ObjectCollections.SizeDecreasingSupplier<K, ObjectOpenHashSet<K>>(expectedSize, (int size) -> size <= Hash.DEFAULT_INITIAL_SIZE ? new ObjectOpenHashSet<K>() : new ObjectOpenHashSet<K>(size)), ObjectOpenHashSet::add, ObjectOpenHashSet::combine, Collector.Characteristics.UNORDERED);
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = size, pos; i-- != 0;) {
			...
			if (((k) == null)) {
				...
				if (!((key[pos = (it.unimi.dsi.fastutil.HashCommon.mix((k).hashCode())) & mask]) == null)) while (!((key[pos = (pos + 1) & mask]) == null));
			}
			key[pos] = k;
		}
		if (ASSERTS) checkTable();
