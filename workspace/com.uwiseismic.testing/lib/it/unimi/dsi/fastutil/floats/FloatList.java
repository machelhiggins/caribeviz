...
public interface FloatList extends List<Float>, Comparable<List<? extends Float>>, FloatCollection {
	...
	default FloatSpliterator spliterator() {
		if (this instanceof java.util.RandomAccess) {
			...
			return FloatSpliterators.asSpliterator(iterator(), sizeOf(this), FloatSpliterators.LIST_SPLITERATOR_CHARACTERISTICS);
		}
	}
	...
	default void replaceAll(final java.util.function.DoubleUnaryOperator operator) {
		replaceAll(operator instanceof FloatUnaryOperator ? (FloatUnaryOperator)operator : x -> it.unimi.dsi.fastutil.SafeMath.safeDoubleToFloat(operator.applyAsDouble(x)));
	}
	 ...
	 */
	public static FloatList of(final float e) {
	 ...
	 */
	public static FloatList of(final float e0, final float e1) {
	 ...
	 */
	public static FloatList of(final float e0, final float e1, final float e2) {
