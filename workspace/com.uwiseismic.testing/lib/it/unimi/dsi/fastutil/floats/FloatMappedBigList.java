...
public class FloatMappedBigList extends AbstractFloatBigList {
	...
	public static int LOG2_BYTES = Long.SIZE - 1 - Long.numberOfLeadingZeros(Float.BYTES);
	/** @deprecated Use {@link #LOG2_BYTES}. */
	@Deprecated
	public static int LOG2_BITS = Long.SIZE - 1 - Long.numberOfLeadingZeros(Float.BYTES);
	 ...
	 */
	public static FloatMappedBigList map(final FileChannel fileChannel, final ByteOrder byteOrder, final MapMode mapMode) throws IOException {
		final long size = fileChannel.size() / Float.BYTES;
		final int chunks = (int)((size + (CHUNK_SIZE - 1)) / CHUNK_SIZE);
		final FloatBuffer[] buffer = new FloatBuffer[chunks];
		for (int i = 0; i < chunks; i++) buffer[i] = fileChannel.map(mapMode, i * CHUNK_SIZE * Float.BYTES, Math.min(CHUNK_SIZE, size - i * CHUNK_SIZE) * Float.BYTES).order(byteOrder).asFloatBuffer();
	...
	public FloatMappedBigList copy() {
		return new FloatMappedBigList(buffer.clone(), size, new boolean[n]);
	}
	...
	public float set(final long index, final float value) {
		final FloatBuffer b = FloatBuffer((int)(index >>> CHUNK_SHIFT));
