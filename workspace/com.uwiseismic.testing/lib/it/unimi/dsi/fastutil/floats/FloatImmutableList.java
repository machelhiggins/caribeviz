...
public class FloatImmutableList extends FloatLists.ImmutableListBase implements FloatList, RandomAccess, Cloneable, java.io.Serializable {
	...
	public int indexOf(final float k) {
		for (int i = 0,
				size = a.length; i < size; i++) if ((Float.floatToIntBits(k) == Float.floatToIntBits(a[i]))) return i;
		return -1;
	}
	...
	public float[] toFloatArray() {
		if (a.length == 0) return FloatArrays.EMPTY_ARRAY;
		return a.clone();
	}
	...
	private final class Spliterator implements FloatSpliterator {
		...
		public int characteristics() {
			return FloatSpliterators.LIST_SPLITERATOR_CHARACTERISTICS | java.util.Spliterator.IMMUTABLE;
		}
